...
int main() {
    ...
    while (g != ITERATIONS) {
        ...
        for (int i = 0; i < N; i++) {
            ...
            field_coordinates[i] =
            ((coordinates[i] - nodes_coordinates[d]) * field_values_nodes[d + 1] / step_for_grid) +
            (coordinates[i] - nodes_coordinates[d + 1]) / step_for_grid * field_values_nodes[d];
